...
import ec.util.*;
import ec.gp.*;
//Used by the Guassian-Newt Algorithm 
import ec.gp.matrix.*;

import java.util.ArrayList;
import java.util.HashMap;
//used for the precoded matrix
...
public class MutateERCPipeline extends GPBreedingPipeline
    ...
    //NOTE TO SELF: To use the MYERC for the standard ERC replace mutateERC ERC variable with MyERC
    public final void mutateERCs(final GPNode node, 
        final EvolutionState state, final int thread)
        {
        // is node an erc?
        if (node instanceof MyERC)
            ((MyERC)node).mutateERC(state,thread);
    ...
    public int produce(final int min,
        ...
        for(int q=start; q < n+start; q++)
            ...
            if (tree==TREE_UNFIXED)
                if (i.trees.length>1) t = state.random[thread].nextInt(i.trees.length); // this line picks a random tree in the GP for the potential mutation to take place.
                else t = 0;
            else t = tree;
