...
public abstract class ERC extends GPNode
    ...
    public void mutateERC(final EvolutionState state, final int thread)
        {
          double v = 0;
          //place these intiliasation back into the paramter section when done -- expect an error in the mutationpipline
          int iteration = 100;
//          double initTemp = 0;
//                  double decayRate = 0;
...
//        while( v < 0.0 && v <= 1.0  ); // this restricition keeps 
//        value = v;
//        resetNode(state,thread);
            
        //Building a Hill Climber
...
//             if (fitness.adjustedFitness() > fitness.adjustedFitness()){ // compare the adjusted old futness with adjusted new fitness 
